...
import rml.args.conversions.map.{AMap, AnEnum, DynMaps}
...
object DefaultLogSetup extends LazyLogging {
  ...
  val loglevel: InputArg[Option[LogLevel.Value]] =
    Opt(AnEnum("level", LogLevel)) -- "Loglevel"
  val initialloglevel: InputArg[LogLevel.Value] =
    AnEnum("initialloglevel", LogLevel) -> LogLevel.INFO -- "Loglevel"
  val loglayout: InputArg[Option[String]] =
    Opt(AMap("layout", logpatterMap)) -- "Loglayout"
  val logpattern: InputArg[Option[String]] =
    Opt(JString("pattern")) -- "Logpattern"
  val initiallogpattern: InputArg[String] =
    JString("initiallogpattern") -> logpatterMap("default") -- "Logpattern"

  val loggers: InputArg[Option[List[Logger]]] = Opt(
    DynMaps("logger", () => LoggerManager.getLoggerMap) -- "Loggers"
  )
  ...
  def apply(prefix: String, systemPrefix: String = "@"): FuncArg[Unit] = {
    ...
    @@(systemPrefix + "log") -->
      Func(loglevel, loglayout, logpattern, loggers) {
        (loglevel, layout, pattern, loggers) =>
          loglevel.foreach { ll =>
            LoggerManager.setLoglevel(
              ll,
              loggers.getOrElse(List(LoggerManager.rootLogger))
            )
          }
