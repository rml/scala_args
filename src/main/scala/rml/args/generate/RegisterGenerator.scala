...
object RegisterGenerator {
  ...
  def generate(maxArgCount: Int): Unit = {
    ...
    for (n <- 0 to maxArgCount) {
      ...
      printf(
        "  def apply[%s](key: String)(desc: String)(%s)(func: %s)(implicit orig: FunctionOrigin): Unit = {\n",
        functype,
        funcargsT,
        functionT
      )
      ...
      printf(
        "  def withoutDescription[%s](key: String)(%s)(func: %s)(implicit orig: FunctionOrigin): Unit = {\n",
        functype,
        funcargsT,
        functionT
      )
