...
object ConfigAdjuster {
  ...
  def apply(
    ...
    val cmdConfig: Config = if (leadingPositionalArgs.isEmpty) {
      ...
      val argsWithPositional: Map[String, List[String]] =
        fullConfig.cmdConfig.args + ("-" -> leadingPositionalArgs)
      fullConfig.cmdConfig.copy(args = argsWithPositional)
    } else {
      val msg =
        "Found leading positional args ([%s] %s) and explicit positional args (-- %s)"
          .format(
            functionName.mkString(" "),
            leadingPositionalArgs.mkString(" "),
            fullConfig.cmdConfig("-").mkString(" ")
          )
  ...
  def adjust(cmdConfig: Config, function: FuncArg[_]): Config = {
    ...
    val trailingArgs = for {
      (k, v) <- cmdLineArgs.toList
    ...
    val legalState =
      trailingArgs.isEmpty || (trailingArgs.tail.isEmpty && !explicitPositionalArgs)
    ...
    if (explicitPositionalArgs) {
      ...
      if (trailingArgs.isEmpty) {
        ...
        val msg =
          "When explicit positional args are set (%s), trailing args are not allowed (%s)"
            .format(
              cmdLineArgs("-").mkString(" "),
              trailingArgs
    ...
    if (trailingArgs.tail.nonEmpty) {
      val msg = "Too many trailing args (%s)".format(
        trailingArgs
          .map { case (k, v) => s"$k: ${v.mkString(" ")}" }
          .mkString(", ")
      )
    ...
    val adjustedArgs = cmdConfig.args ++ Map(
      adjustArgName -> adjustArg.getUsed(
        argList
      ),
      "-" -> adjustArg.getUnused(argList)
    )
