...
object CommandlineArgReader extends RegexParsers {
  ...
  private def command: Parser[CmdLineArgs] = functionName~rep(functionName)~arguments ^^ {	
    case func~subfuncs~args => CmdLineArgs(func, subfuncs, args.toMap, if(args.isEmpty) "" else args.last._1)
  }
