...
object FunctionRegister extends Logging {
   ...
   */
  private val register = scala.collection.mutable.Map[List[String], FuncArg[_]]()
  
  def commands(sep: String = " "): List[String] = register.keySet.map(_.mkString(sep)).toList.sorted

  def list(filter: String): List[(String, FuncArg[_])] = register.filter{case(k, v) => k.mkString(" ") contains filter}.map{case(k, v) => (k.mkString(" "), v)}.toList.sortBy(_._1)
  ...
  def get(key: List[String]): FuncArg[_] = {
    ...
    findLongestMatching(key) match {
      ...
      case None => throw new IllegalArgException("No function defined for key: '" + key.mkString(" ") + "'")
    }
  }
   ...
   * Convenience function
   */
  def update(key: List[String], fdef: FuncArg[_]) = set(key, fdef)
  
  /**
   * Convenience function
   */
  def update(key: String, fdef: FuncArg[_]) = set(key :: Nil, fdef)
