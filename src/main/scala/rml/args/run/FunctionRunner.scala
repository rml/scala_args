...
import rml.args.config.{ConfigAdjuster, FullConfig}
...
object FunctionRunner extends Logging {
  ...
  def run(fullConfig: FullConfig): Try[Any] = {
    ...
    val functionName =
      FunctionRegister.findLongestMatching(leadingTokens) match {

        case Some(k) => k
        case None =>
          return Failure(new FunctionNotFoundException(leadingTokens))
      }
    ...
    val adjustedConfig: FullConfig =
      ConfigAdjuster(fullConfig, function, functionName)
