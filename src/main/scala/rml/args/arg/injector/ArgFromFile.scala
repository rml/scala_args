...
import rml.args.arg.{Arg, FuncArg, InputArg, Mapper}
import rml.args.arg.decorator.{FileInjector, ListFileInjector}
import rml.args.config.FullConfig
import rml.args.conversions.strings.{AString, Strings}
...
class ArgMapFromFile[T](resultArg: Arg[T]) {
  ...
  def apply(key: String, files: Arg[List[File]]): FuncArg[Map[String, T]] = {
    ...
    val mapper = Mapper[Map[String, T]](files) { config: FullConfig =>
      def csvEntryToArg(
          csvEntry: Map[String, List[String]]
      ): Try[(String, T)] = {
      ...
      def fileToArgMap(file: File): Map[String, T] = {
        ...
        val successEntries =
          CsvReader.rowsAsMaps(file).map(csvEntryToArg).filter(_.isSuccess)
