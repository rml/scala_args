...
import rml.args.arg.restriction.{FixRestriction, NoRestriction, Restriction}
...
final case class InputArg[+T](
    key: String,
    mapper: InputMapper[T],
    argState: ArgState = ArgState(),
    defaultArgs: List[Arg[T]] = List.empty
) extends Arg[T] {
  ...
  def defaultArgValue(config: FullConfig): Try[T] = {
    ...
    defaultArgs
      ...
      .find(_.isSuccess)
      .getOrElse(
        throw new IllegalArgException("No default argument for key " + key)
      )
  }
  ...
  override def apply(config: FullConfig): Try[T] = {
    ...
    val withDefault =
      if (defaultArgs.isEmpty) mainArg
      else mainArg.orElse(defaultArgValue(config))

    withDefault.orElse(throw new IllegalArgException("No value for key " + key))
  }

  private def mkMapper[S](
      applyFunc: (FullConfig, String, List[String]) => Try[S]
  ): InputMapper[S] = {
  ...
  private def mkMapper[S](
      mapperName: String
  )(applyFunc: (FullConfig, String, List[String]) => Try[S]): InputMapper[S] = {

    new InputMapper[S] {

      override def restriction: Restriction = mapper.restriction

      override def getUnused(argList: List[String]): List[String] =
        mapper.getUnused(argList)

      override def getUsed(argList: List[String]): List[String] =
        mapper.getUsed(argList)
  ...
  def mapLowLevel[S](
      mapperName: String
  )(func: (InputArg[T], FullConfig) => Try[S]): InputArg[S] = {
  ...
  def mapLowLevel[S](func: (InputArg[T], FullConfig) => Try[S]): InputArg[S] =
    mapLowLevel("mapper")(func)
  ...
  def withAlias(aliases: String*): InputArg[T] =
    copy(argState = argState.copy(aliases = aliases.toList))
  def ~(aliases: String*): InputArg[T] = withAlias(aliases: _*)
  def +(aliases: String*): InputArg[T] = withAlias(aliases: _*)

  def withDefault[X >: T](defaultArgs: Arg[X]*): InputArg[X] =
    copy(defaultArgs = defaultArgs.toList)
  def ->[X >: T](defaultArgs: Arg[X]*): InputArg[X] = withDefault(
    defaultArgs: _*
  )
...
//  def        -> [X >: T](default: X): InputArg[X] = withDefault(default)
  ...
  def withRestriction(restriction: Restriction): InputArg[T] =
    copy(argState = argState.copy(restriction = restriction))
  def |->(restriction: Restriction): InputArg[T] = withRestriction(restriction)

  def withRestriction(inputValues: List[String]): InputArg[T] = withRestriction(
    FixRestriction(inputValues.toSet)
  )
