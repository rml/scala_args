...
object Opt {
  ...
  def apply[T](arg: InputArg[T]): InputArg[Option[T]] = arg.mapLowLevel("arg -> Opt(arg)"){ (inputArg, config) =>
  ...
  def apply[T](arg: FuncArg[T]): FuncArg[Option[T]] = arg.mapLowLevel{ (funcArg, config) =>
