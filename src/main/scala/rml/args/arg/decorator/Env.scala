...
object Env {
  ...
  def apply[T](arg: InputArg[T]): InputArg[T] = arg.mapLowLevel {
    (arg: InputArg[T], config: FullConfig) =>
      val envVar = System.getenv(arg.key)

      if (envVar == null) {
        Failure(
          new IllegalArgException(
            s"No values found for environment variable ${arg.key} found"
          )
        )
      } else {
        val configWithEnv =
          config.over(arg.key, CommandlineArgReader.parseArgumentValues(envVar))
