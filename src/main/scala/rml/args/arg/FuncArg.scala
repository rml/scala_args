...
final case class FuncArg[+T](
  ...
  override def map[S](func: T => S): FuncArg[S] = {
    FuncArg(Mapper { config => this.apply(config).map(func) })
  }

  override def flatMap[S](func: T => Try[S]): FuncArg[S] = {
    FuncArg(Mapper { config => this.apply(config).flatMap(func) })
  }

  def mapLowLevel[S](func: (FuncArg[T], FullConfig) => Try[S]): FuncArg[S] = {
    FuncArg(Mapper { config => func(this, config) })
  }
  ...
  def withDefault[X >: T](defaultArgs: Arg[X]*): FuncArg[X] =
    copy(defaultArgs = defaultArgs.toList)
  def ->[X >: T](defaultArgs: Arg[X]*): FuncArg[X] = withDefault(
    defaultArgs: _*
  )
  ...
  def withDescription(desc: String): FuncArg[T] =
