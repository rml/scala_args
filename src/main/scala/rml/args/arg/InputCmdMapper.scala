...
trait InputCmdMapper[+T] extends InputMapper[T] with LazyLogging {
  ...
  def stringArgs(config: FullConfig, key: String, aliases: List[String]) = {
  ...
  def apply(config: FullConfig, key: String, aliases: List[String]): Try[T] = {
    ...
    stringArgsTry.map(getUnused).filter(!_.isEmpty).foreach(unused => handleUnusedValues(key, unused: List[String]))
    
    stringArgsTry.map(getUsed).map(mapListToType)
  }
  
  def handleUnusedValues(key: String, unused: List[String]): Unit = {
    logger.warn(s"Found unused trailing value(s) for argument $key: " +  unused.mkString("'", "', '", "'"))
  }
